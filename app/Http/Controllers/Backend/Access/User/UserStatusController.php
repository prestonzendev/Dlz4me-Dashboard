...
class UserStatusController extends Controller
{
    ...
    public function mark(User $user, $status, EditUserRequest $request)
    {
        ...
        if($user->roles[0]->name == "Customer"){
            return redirect()->route($status == 1 ? 'admin.access.user.customers.index' : 'admin.access.user.deactivated')->withFlashSuccess(trans('alerts.backend.users.updated'));
        }
        else
        {
           return redirect()->route($status == 1 ? 'admin.access.user.escorts.index' : 'admin.access.user.deactivated')->withFlashSuccess(trans('alerts.backend.users.updated'));
       }
     ...
     * @return mixed
     */
    public function softDelete(User $deletedUser, DeleteUserRequest $request)
    {
        $user = User::with('role')->find($deletedUser->id);
        ...
        return redirect()->route('admin.access.user.customers.index')->withFlashSuccess(trans('alerts.backend.users.deleted'));
    }
