...
class UserController extends Controller
{
    ...
    public function update(User $user, UpdateUserRequest $request)
    {
        ...
        if ($user->roles[0]->name == 'Customer') {
            ...
            return new RedirectResponse(route('admin.access.user.vendor.index'), ['flash_success' => trans('alerts.backend.users.updated')]);
        }
    ...
    public function destroy(User $user, DeleteUserRequest $request)
    {
        if ($user->roles[0]->name == 'Customer') {
            ...
            return new RedirectResponse(route('admin.access.user.customers.index'), ['flash_success' => trans('alerts.backend.users.deleted')]);
        } else {
            $this->users->delete($user);
            return new RedirectResponse(route('admin.access.user.escorts.index'), ['flash_success' => trans('alerts.backend.users.deleted')]);
        }
    }
}
