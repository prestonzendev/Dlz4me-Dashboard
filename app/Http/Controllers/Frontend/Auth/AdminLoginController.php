...
use App\Traits\AuthenticateAdminLogin;
...
class AdminLoginController extends Controller
{
    ...
    {
        $this->middleware('guest:admin')->except(['logout','logoutAs']);
    ...
    {
        $email = \Cookie::get('eageradmin');
        return view('frontend.auth.admin_login')->with([
            'email' => $email,
        ]);
    }
    ...
    protected function authenticated(Request $request, $user)
    {
        ...
        if (isset($inputs['remember'])) {
            \Cookie::queue('eageradmin', $inputs['email'], 1440);
        } else {
            \Cookie::queue('eageradmin', $inputs['email'], -1);
        }
        /*
