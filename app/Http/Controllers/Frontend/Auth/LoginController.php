...
class LoginController extends Controller
{
    ...
    {
        $this->middleware('guest:user')->except(['logout','logoutAs']);
    ...
    public function redirectPath()
    {
        ...
        if($this->guard()->user()->roles()->first()->name == "Vendor"){ 
            return route('frontend.vendor.dashboard');
        }elseif($this->guard()->user()->roles()->first()->name == "Customers"){  
    ...
    protected function authenticated(Request $request, $user){
        ...
        if (isset($inputs['remember'])) {
            \Cookie::queue('eager_user', $inputs['email'], 1440);
        } else {
            \Cookie::queue('eager_user', $inputs['email'], -1);
        }
